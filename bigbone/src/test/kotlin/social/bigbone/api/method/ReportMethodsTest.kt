...
class ReportMethodsTest {
    ...
    fun reportFile() {
        ...
        with(result) {
            ...
            actionTakenAt shouldBeEqualTo Unavailable
            createdAt shouldBeEqualTo ExactTime(Instant.parse("2022-08-25T09:56:16.763Z"))
        }

        with(result.targetAccount) {
            shouldNotBeNull()
            createdAt shouldBeEqualTo ExactTime(Instant.parse("2022-05-26T00:00:00.000Z"))
            lastStatusAt shouldBeEqualTo StartOfDay(
                LocalDate.parse("2022-08-25").atStartOfDay(ZoneOffset.UTC).toInstant()
            )
        }
    }
