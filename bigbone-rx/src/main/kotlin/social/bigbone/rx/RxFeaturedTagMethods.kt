...
class RxFeaturedTagMethods(client: MastodonClient) {
     ...
     */
    fun getFeaturedTags(): Single<List<FeaturedTag>> = Single.fromCallable { featuredTagsMethods.getFeaturedTags().execute() }
     ...
     */
    fun featureTag(tagName: String): Single<FeaturedTag> = Single.fromCallable { featuredTagsMethods.featureTag(tagName).execute() }
