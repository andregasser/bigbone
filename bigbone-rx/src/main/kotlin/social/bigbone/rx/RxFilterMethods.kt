...
class RxFilterMethods(client: MastodonClient) {
    ...
    ): Single<Filter> = Single.fromCallable { filterMethods.createFilter(title, context, filterKeywords, expiresIn, filterAction).execute() }
     ...
     */
    fun listKeywords(filterId: String): Single<List<FilterKeyword>> = Single.fromCallable { filterMethods.listKeywords(filterId).execute() }
    ...
    ): Single<FilterKeyword> = Single.fromCallable { filterMethods.addKeyword(filterId, filterKeyword).execute() }
     ...
     */
    fun viewKeyword(keywordId: String): Single<FilterKeyword> = Single.fromCallable { filterMethods.viewKeyword(keywordId).execute() }
     ...
     */
    fun updateKeyword(filterKeyword: FilterKeyword): Single<FilterKeyword> = Single.fromCallable { filterMethods.updateKeyword(filterKeyword).execute() }
     ...
     */
    fun deleteKeyword(keywordId: String): Completable = Completable.fromAction { filterMethods.deleteKeyword(keywordId) }
     ...
     */
    fun listStatusFilters(filterId: String): Single<List<FilterStatus>> = Single.fromCallable { filterMethods.listStatusFilters(filterId).execute() }
    ...
    fun addStatusToFilter(
        filterId: String,
        statusId: String
    ): Single<FilterStatus> = Single.fromCallable { filterMethods.addStatusToFilter(filterId, statusId).execute() }
     ...
     */
    fun viewStatusFilter(filterStatusId: String): Single<FilterStatus> = Single.fromCallable { filterMethods.viewStatusFilter(filterStatusId).execute() }
