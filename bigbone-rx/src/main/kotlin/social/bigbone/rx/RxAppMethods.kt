...
class RxAppMethods(client: MastodonClient) {
    ...
    ): Single<Application> = Single.fromCallable { appMethods.createApp(clientName, redirectUris, website, scope).execute() }
     ...
     */
    fun verifyCredentials(): Single<Application> = Single.fromCallable { appMethods.verifyCredentials().execute() }
}
