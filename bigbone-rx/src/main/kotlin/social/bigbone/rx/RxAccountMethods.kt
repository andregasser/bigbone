...
class RxAccountMethods(client: MastodonClient) {
     ...
     */
    fun getAccount(accountId: String): Single<Account> = Single.fromCallable { accountMethods.getAccount(accountId).execute() }
     ...
     */
    fun verifyCredentials(): Single<Account> = Single.fromCallable { accountMethods.verifyCredentials().execute() }
    ...
    fun updateCredentials(
        ...
        avatar: String?,
        header: String?
    ): Single<Account> = Single.fromCallable { accountMethods.updateCredentials(displayName, note, avatar, header).execute() }
    ...
    fun getFollowers(
        accountId: String,
        range: Range = Range()
    ): Single<Pageable<Account>> = Single.fromCallable { accountMethods.getFollowers(accountId, range).execute() }
    ...
    fun getFollowing(
        accountId: String,
        range: Range = Range()
    ): Single<Pageable<Account>> = Single.fromCallable { accountMethods.getFollowing(accountId, range).execute() }
     ...
     */
    fun followAccount(accountId: String): Single<Relationship> = Single.fromCallable { accountMethods.followAccount(accountId).execute() }
     ...
     */
    fun unfollowAccount(accountId: String): Single<Relationship> = Single.fromCallable { accountMethods.unfollowAccount(accountId).execute() }
     ...
     */
    fun blockAccount(accountId: String): Single<Relationship> = Single.fromCallable { accountMethods.blockAccount(accountId).execute() }
     ...
     */
    fun unblockAccount(accountId: String): Single<Relationship> = Single.fromCallable { accountMethods.unblockAccount(accountId).execute() }
     ...
     */
    fun muteAccount(accountId: String): Single<Relationship> = Single.fromCallable { accountMethods.muteAccount(accountId).execute() }
     ...
     */
    fun unmuteAccount(accountId: String): Single<Relationship> = Single.fromCallable { accountMethods.unmuteAccount(accountId).execute() }
     ...
     */
    fun getRelationships(accountIds: List<String>): Single<List<Relationship>> = Single.fromCallable { accountMethods.getRelationships(accountIds).execute() }
    ...
    fun searchAccounts(
        query: String,
        limit: Int? = null
    ): Single<List<Account>> = Single.fromCallable { accountMethods.searchAccounts(query, limit).execute() }
}
