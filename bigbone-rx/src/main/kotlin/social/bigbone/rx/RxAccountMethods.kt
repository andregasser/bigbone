...
class RxAccountMethods(client: MastodonClient) {
    ...
    fun getAccount(accountId: String): Single<Account> =
        Single.fromCallable { accountMethods.getAccount(accountId).execute() }
    ...
    fun followAccount(accountId: String): Single<Relationship> =
        Single.fromCallable { accountMethods.followAccount(accountId).execute() }

    fun unfollowAccount(accountId: String): Single<Relationship> =
        Single.fromCallable { accountMethods.unfollowAccount(accountId).execute() }

    fun blockAccount(accountId: String): Single<Relationship> =
        Single.fromCallable { accountMethods.blockAccount(accountId).execute() }

    fun unblockAccount(accountId: String): Single<Relationship> =
        Single.fromCallable { accountMethods.unblockAccount(accountId).execute() }

    fun muteAccount(accountId: String): Single<Relationship> =
        Single.fromCallable { accountMethods.muteAccount(accountId).execute() }

    fun unmuteAccount(accountId: String): Single<Relationship> =
        Single.fromCallable { accountMethods.unmuteAccount(accountId).execute() }

    fun getRelationships(accountIds: List<String>): Single<List<Relationship>> =
        Single.fromCallable { accountMethods.getRelationships(accountIds).execute() }
