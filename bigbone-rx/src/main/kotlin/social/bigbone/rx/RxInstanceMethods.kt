...
class RxInstanceMethods(client: MastodonClient) {
     ...
     */
    fun getPeers(): Single<List<String>> = Single.fromCallable {
        instanceMethods.getPeers().execute()
    }
     ...
     */
    fun getActivity(): Single<List<InstanceActivity>> = Single.fromCallable {
        instanceMethods.getActivity().execute()
    }
     ...
     */
    fun getRules(): Single<List<Rule>> = Single.fromCallable {
        instanceMethods.getRules().execute()
    }
     ...
     */
    fun getBlockedDomains(): Single<List<DomainBlock>> = Single.fromCallable {
        instanceMethods.getBlockedDomains().execute()
    }
     ...
     */
    fun getExtendedDescription(): Single<ExtendedDescription> = Single.fromCallable {
        instanceMethods.getExtendedDescription().execute()
    }
}
